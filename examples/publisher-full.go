...
//
//	LOG_LEVEL=debug go run ./examples/publisher-full.go
func main() {
	...
	provider := rmq.Provider{
		...
		Publish: &rmq.PublishConfig{
			...
			Immediate: false,
			// Confirm, if true, puts the channel into confirm mode so Publish waits
			// for a broker acknowledgement before returning.
			// Use this when delivery guarantees matter.
			Confirm:   true,
			Arguments: amqp.Table{},
		},
	}
	...
	defer func() {
		if err := svc.Close(); err != nil {
			logger.Error("rmq: close failed", "error", err)
		}
	}()
	...
	if err := svc.Publish(context.Background(), []byte("hello")); err != nil {
